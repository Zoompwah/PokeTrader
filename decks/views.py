...
def edit_deck(request, deck_id):
    ...
    return render(request, 'edit_deck.html', {'deck': deck})

@login_required
def delete_card(request, deck_id, card_id):
    deck = get_object_or_404(Deck, pk=int(deck_id))
    card = get_object_or_404(Card, pk=int(card_id))

    if not DeckCard.objects.filter(deck=deck, card=card).exists():
        messages.error(request, "The specified card does not exist in this deck.")
        return redirect('view_deck_details', deck_id=deck_id)
    
    DeckCard.objects.filter(deck=deck, card=card).delete()
    messages.success(request, f"The card '{card.name}' has been successfully removed from the deck.")
...
def add_card_to_deck(request, deck_id):
    ...
    if request.method == 'POST':
        ...
        for card_id in deck_cards:
            card = get_object_or_404(Card, pk=int(card_id))
            deck_cards, created = DeckCard.objects.get_or_create(deck=deck, card=card)
            if created:
                messages.success(request, f"The card '{card.name}' has been successfully added to the deck.")
            else:
                messages.warning(request, f"The card '{card.name}' is already in the deck.")
        
        return redirect('view_deck_details', deck_id=deck_id)
    
    return render(request, 'add_card.html', {'deck': deck, 'cards': Card.objects.filter(collection__user=request.user.id)})